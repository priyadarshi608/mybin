...
# List of NIFTY50 tickers (Add more or replace with actual list)
NIFTY50_TICKERS = [
    "ADANIENT.NS",
    "ADANIPORTS.NS",
    "APOLLOHOSP.NS",
        ...
        if trend == 'Uptrend':
            ...
            if win_rate >= 0.8 and risk_reward_ratio >= 2:
                return {
                    ...
                    "reason": f"Stock is in uptrend (RSI={round(rsi, 2)}, MA20={round(ma20, 2)}, MA50={round(ma50, 2)}), "
                              f"favorable risk-reward ratio ({risk_reward_ratio}), and estimated win rate of {win_rate}%."
                }
        return None
...
def get_short_term_nifty50_recommendations():
    """Returns short-term NIFTY50 stock recommendations with 2% target, 1% stoploss, and ≥80% win rate."""
    results = []
    ...
    if not results:
        return {"message": "No stocks found matching the criteria."}
...
# Finance Agent with custom recommendation tool
finance_agent = Agent(
    name="Short-Term Stock Advisor",
    model=groq_model,
    tools=[get_short_term_nifty50_recommendations],
    instructions=[
        ...
        "For each recommendation, explain why it's being suggested.",
