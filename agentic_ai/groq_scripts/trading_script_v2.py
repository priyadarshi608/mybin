...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'trading_script_v2.log')
    ]
)
...
try:
    ...
    def get_pre_features(row, conn):
        ...
        more_past_returns = additional_past_returns(df_past, 60)
    ...
    def related_stock_data(symbol, df_past, conn, trade_date_str):
        query = f"SELECT * FROM market_data WHERE symbol='{symbol}' AND duration='1d' AND date(start_timestamp) < '{trade_date_str}' ORDER BY start_timestamp DESC LIMIT 30"
    ...
    for idx, row in df_trades.iterrows():
        ...
        if feats is None:
            features_list.append({k: np.nan for k in ['rsi', 'ret1', 'ret5', 'vol', 'ret1_index', 'time_frac', 'macd', 'more_past_returns', 'icici_data', 'hdfc_data', 'sensex_data']})
        else:
    ...
    profitable_trades = df_adapted[df_adapted['pp'] >= 0]
    avg_pp = profitable_trades['pp'].mean()
    ...
    for idx, row in df_adapted.iterrows():
        ...
        val = (
            row['symbol'], row['trade_date'], 2, row['open_price'], row['position_price'], row['position_timestamp'],
