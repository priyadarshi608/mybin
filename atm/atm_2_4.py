...
variance_rolling_window = 180
garch_input_window = 180
...
for symbol_name in symbols:
    ...
    # # === INSERT ALL DAILY SIGNALS (NaN → 0) INTO daily_symbol_signals ===
    # signals_to_save = daily_df[['signal_daily']].reset_index()
    # signals_to_save['signal_generation_date'] = signals_to_save['Date'].dt.date
    # signals_to_save['signal_for_next_day'] = signals_to_save['signal_daily'].fillna(0).astype(int)
    # signals_to_save['symbol'] = symbol_name
    # signals_to_save = signals_to_save[['symbol', 'signal_generation_date', 'signal_for_next_day']]

    # if not signals_to_save.empty:
    #     signals_to_save.to_sql(
    #         name='daily_symbol_signals',
    #         con=engine,
    #         if_exists='append',
    #         index=False
    #     )
    ...
    for date, group in temp_df.groupby(temp_df['datetime'].dt.date):
        ...
        # ✅ Move entry to next candle’s open
